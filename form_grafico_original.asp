			...
			<table  class="table table-bordered">
				...
				<tbody>
					...
					if NOT RESULTADO_SQL_1.EOF Then	
						...
						Do Until RESULTADO_SQL_1.EOF
							...
							<tr>
								...
								<td><button data-toggle="modal" data-target="#Modal-<%=VAR_SQL_1_COD%>" type="button" class="btn btn-primary"><span class="glyphicon glyphicon-signal"></span> Grafico</button></td>
								
								<%If session("funcao") = "INSPETOR" or session("funcao") = "SUPERVISOR" or session("nivel") = "ADM" Then%>
								<td><i style='cursor:pointer;color:red;' onclick='excluir(this.id)' id="<%=VAR_SQL_1_COD%>" class='glyphicon glyphicon-remove' style='font-size:18px;'>Excluir</i></td>
								<%end if%>
							</tr>
					        <%
					...
					function excluir(value)
					{
						...
						var r=confirm("Você deseja realmente excluir esse registro?");
						...
						if (r==true)
						{
							...
							$.ajax(
							{
								...
								//Pagina que recebe os daods
								 url: "valida_excluir_registro.asp",
